...
void InputManager::AssignEvent(SDL_EventType evt, InputEvent callback)
{
	...
	{
		std::vector<InputBlock *> vectorBlock(false,newBlock);
		    std::pair<SDL_EventType, std::vector<InputBlock *> > newPair(evt, vectorBlock);
...
void InputManager::AssignEvent(SDL_EventType evt, void *target, InputMemberEvent callback)
{
	...
	if (mapItr == _memberEventMap.end())
	{
		...
		std::vector<InputPair *> vectorPair(false,newPair) ;  
		std::pair<SDL_EventType, std::vector<InputPair *> > newEntry(evt, vectorPair);
